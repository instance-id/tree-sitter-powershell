...
module.exports = grammar({
  ...
  rules: {
    ...
    path_command_name: ($) =>
      prec.right(repeat1(choice($.path_command_name_token, $.variable))),
    ...
    foreach_command: ($) =>
      seq(
        choice("%", reservedWord("foreach-object")),
    ...
    // Class
    class_attribute: ($) =>
      choice(reservedWord("hidden"), reservedWord("static")),
...
function reserved(regex) {
  // For operators starting with '-', ensure they're standalone tokens
  if (regex.startsWith("-")) {
    return token(prec(PREC.KEYWORD, new RegExp(`${regex}(?![a-zA-Z0-9_])`)));
  }
  // For normal keywords, use word boundaries
  return token(prec(PREC.KEYWORD, new RegExp(`\\b${regex}\\b`)));
}
